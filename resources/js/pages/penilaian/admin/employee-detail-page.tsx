...
import { ArrowLeft, Award, BarChart3, Calculator, Download, FileText, Home, Share2, Users } from 'lucide-react';
...
export default function EmployeeDetailPage() {
    ...
    const chartData = employee.evaluatorScores.map((evaluator) => ({
        name: evaluator.type === 'atasan' ? 'Atasan' : evaluator.type === 'penerima_layanan' ? 'Penerima Layanan' : 'Outsourcing',
        evaluatorName: evaluator.evaluatorName,
        'Nilai Total': evaluator.overallScore,
        weight: evaluator.weight * 100,
        weightedScore: evaluator.overallScore * evaluator.weight,
    }));

    // Prepare line chart data for 3 evaluators comparison
    const lineChartData = [
        {
            name: 'Aspek Teknis',
            Atasan: employee.evaluatorScores.find((e) => e.type === 'atasan')?.aspectScores['aspek-teknis'] || 0,
            'Penerima Layanan': employee.evaluatorScores.find((e) => e.type === 'penerima_layanan')?.aspectScores['aspek-teknis'] || 0,
            Outsourcing: employee.evaluatorScores.find((e) => e.type === 'outsourcing')?.aspectScores['aspek-teknis'] || 0,
        },
        {
            name: 'Aspek Perilaku',
            Atasan: employee.evaluatorScores.find((e) => e.type === 'atasan')?.aspectScores['aspek-perilaku'] || 0,
            'Penerima Layanan': employee.evaluatorScores.find((e) => e.type === 'penerima_layanan')?.aspectScores['aspek-perilaku'] || 0,
            Outsourcing: employee.evaluatorScores.find((e) => e.type === 'outsourcing')?.aspectScores['aspek-perilaku'] || 0,
        },
    ];
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                        <TabsList className="mb-5 grid w-full grid-cols-4 rounded-lg border border-gray-200 bg-white p-1 shadow-sm">
                            ...
                            >
                                ...
                                <span className="hidden sm:inline">Per Evaluator</span>
                            </TabsTrigger>
                            <TabsTrigger
                                value="peraspek"
                                className="flex items-center space-x-2 data-[state=active]:bg-indigo-500 data-[state=active]:text-white"
                            >
                                ...
                                <span className="hidden sm:inline">Per Aspek</span>
                            </TabsTrigger>
                            <TabsTrigger
                                value="detail"
                                className="flex items-center space-x-2 data-[state=active]:bg-indigo-500 data-[state=active]:text-white"
                        ...
                        <TabsContent value="chart" className="space-y-6">
                            ...
                            {/* Bar Chart - Perbandingan Nilai dari 3 Penilai - TOTAL SCORES ONLY */}
                            <Card>
                                <CardHeader>
                                    <CardTitle>Perbandingan Nilai dari 3 Penilai</CardTitle>
                                    <CardDescription>
                                        Visualisasi nilai total dari setiap penilai dengan informasi bobot masing-masing
                                    </CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <div className="h-96">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                                ...
                                                />
                                                <Legend />
                                                <Bar dataKey="Nilai Total" fill="#8B5CF6" name="Nilai Total" />
                                            </BarChart>
                                        </ResponsiveContainer>
                                    </div>

                                    {/* Summary Cards for Total Scores */}
                                    <div className="mt-6 grid grid-cols-3 gap-4">
                                        <div className="rounded-lg border-2 border-red-200 bg-red-50 p-4 text-center">
                                            <div className="mb-1 text-sm text-red-600">Atasan</div>
                                            <div className="text-3xl font-bold text-red-700">
                                                {employee.evaluatorScores.find((e) => e.type === 'atasan')?.overallScore.toFixed(1) || 0}
                                            </div>
                                            <div className="text-xs text-red-500">Bobot: 50%</div>
                                        </div>
                                        <div className="rounded-lg border-2 border-blue-200 bg-blue-50 p-4 text-center">
                                            <div className="mb-1 text-sm text-blue-600">Penerima Layanan</div>
                                            <div className="text-3xl font-bold text-blue-700">
                                                {employee.evaluatorScores.find((e) => e.type === 'penerima_layanan')?.overallScore.toFixed(1) || 0}
                                            </div>
                                            <div className="text-xs text-blue-500">Bobot: 30%</div>
                                        </div>
                                        <div className="rounded-lg border-2 border-green-200 bg-green-50 p-4 text-center">
                                            <div className="mb-1 text-sm text-green-600">Outsourcing</div>
                                            <div className="text-3xl font-bold text-green-700">
                                                {employee.evaluatorScores.find((e) => e.type === 'outsourcing')?.overallScore.toFixed(1) || 0}
                                            </div>
                                            <div className="text-xs text-green-500">Bobot: 20%</div>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                            ...
                            <Card className="gap-0 py-0 shadow-xl">
                                <CardHeader className="bg-gradient-to-r from-gray-50 to-blue-50 py-5 pl-7">
                                    ...
                                    <CardDescription className="text-gray-600">
                                        Insight kualitatif dan masukan konstruktif dari masing-masing penilai untuk pengembangan kinerja
                                    </CardDescription>
                                </CardHeader>
                                <CardContent className="p-8">
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="mb-4 flex items-center justify-between">
                                                            ...
                                                            <div className="text-center">
                                                                <div className="mb-1 text-sm text-blue-600">Bobot</div>
                                                        ...
                                                        <div className="mb-4 rounded-lg bg-white p-4 text-center">
                                                            <div className="mb-2 text-sm text-blue-600">Nilai Berbobot</div>
                                                        ...
                                                        <div className="flex items-center justify-between rounded-sm bg-white p-3 text-sm">
                                                            <span className="text-gray-600">Nilai Inputan:</span>
                                                        ...
                                                        {/* Formula */}
                                                        <div className="mt-3 text-center font-mono text-xs text-blue-600">
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <div className="mb-4 flex items-center justify-between">
                                                            ...
                                                            <div className="text-center">
                                                                <div className="mb-1 text-sm text-green-600">Bobot</div>
                                                        ...
                                                        {/* Weighted Score - MOST PROMINENT */}
                                                        <div className="mb-4 rounded-lg bg-white p-4 text-center">
                                                            <div className="mb-2 text-sm text-green-600">Nilai Berbobot</div>
                                                            <div className="text-4xl font-bold text-red-500">{weightedScore.toFixed(2)}</div>
                                                        ...
                                                        {/* Formula */}
                                                        <div className="mt-3 text-center font-mono text-xs text-green-600">
                                                            {originalScore.toFixed(1)} × {(evaluator.weight * 100).toFixed(0)}% ={' '}
                                                            {weightedScore.toFixed(2)}
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-6">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="mt-3 text-center font-mono text-xs text-gray-600">
                                                                {evaluator.aspectScores['aspek-teknis'].toFixed(1)} × {evaluator.weight.toFixed(0)}% ={' '}
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="mt-3 text-center font-mono text-xs text-gray-600">
                                                                {evaluator.aspectScores['aspek-perilaku'].toFixed(1)} × {evaluator.weight.toFixed(0)}%
